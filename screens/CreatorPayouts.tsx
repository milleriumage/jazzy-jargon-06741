...
const CreatorPayouts: React.FC = () => {
    const { earnedBalance, creatorTransactions, devSettings, withdrawalTimeEnd, processWithdrawal } = useCredits();
    ...
    const handleWithdrawal = async () => {
        if (!isWithdrawalReady || isProcessing) return;
        ...
        if (success) {
            alert(`Withdrawal request submitted for $${(earnedBalance * devSettings.creditValueUSD).toFixed(2)}`);
        } else {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            ...
            <div className="bg-neutral-800 rounded-lg p-6">
                ...
                >
                    <PayoutsIcon />
                    <span className="ml-2">{isProcessing ? 'Processing...' : `Withdraw $${(earnedBalance * devSettings.creditValueUSD).toFixed(2)}`}</span>
                </button>
                    ...
                    {creatorTransactions.length > 0 ? (
                        <ul className="divide-y divide-neutral-700">
                            {creatorTransactions.map((tx: CreatorTransaction) => {
                                const buyer = useCredits().allUsers.find(u => u.id === tx.buyerId);
                                return (
                                <li key={tx.id} className="p-4 flex justify-between items-center">
                                    <div>
                                        ...
                                        <p className="text-sm text-neutral-400">
                                            Purchased by {buyer?.username || 'Unknown User'} on {new Date(tx.timestamp).toLocaleDateString()}
                                        </p>
                                    </div>
